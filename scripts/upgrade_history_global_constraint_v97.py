...
if 'def confirmed_roster_one_side_pass():' not in py or 'def confirmed_roster_club(ids):' not in py:
    raise RuntimeError('v96 one-side confirmed-roster decoder must exist before v97')
old="    def single_side_bridge_pass():\n"
new="""    def confirmed_roster_global_constraint_pass():
...
for s in ['def confirmed_roster_global_constraint_pass():','2<=len(uniq)<=8','if len(cs)!=len(fs):','perfect_matching(cs,(ci,fk))',"'unlabelled_retained_confirmed_roster_global_unique'",'g=confirmed_roster_global_constraint_pass()','unlabelled_rich_global_constraint_fixture_matches']:assert s in cpy,s
print('v97 globally unique retained fixture recovery applied')
