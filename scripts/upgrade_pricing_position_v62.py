...
def main():
    ...
    # v6.1's exception was based on CA beating the actual starter. That let a zero-minute
    # reserve bypass the usage caps. v6.2 permits the exception only for a structurally
    # decoded current injury/suspension, which is now available before pricing runs.
    pat=re.compile(r"(?P<indent>^[ \t]*)expected_exception=bool\(games>=2 and mins==0 and d>=0\.78 and q>=0\.78 and ca>=active_ca\+4\)$",re.M)
    def repl(mm):
        i=mm.group('indent')
        return (i+"absence_excused=bool(p.get('injured') or p.get('suspended') or p.get('injury_status')=='injured' or p.get('suspension_status')=='suspended')\n"+
    ...
    # A verified absent zero-minute player is not basement fodder, but also cannot be
    # promoted into the premium tier solely by CA/depth reputation.
    ...
    # Use actual played-role evidence to resolve DEF/MID hybrids. We still self-calibrate
    # marker meanings from the save and demand repeated 75%+ usage before reclassification.
