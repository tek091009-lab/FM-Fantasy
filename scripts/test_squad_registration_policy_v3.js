...
function context(extra={}){const c={console,setTimeout:()=>0,clearTimeout:()=>{},setInterval:()=>0,clearInterval:()=>{},...extra};c.window=c.window||{addEventListener:()=>{}};c.globalThis=c;return vm.createContext(c)}
{
 ...
 const payload={meta:{unresolved_squad_eids:[],ambiguous_memberships:[{player_eid:9}]},fixtures:[{home:'A',away:'B'}],players:[
  {id:1,name:'Registered',club:'A',competition_eligible:true,registration_status:'competition_eligible',registration_evidence:{eligible:true},history:[],visible:true},
  {id:2,name:'Senior evidence',club:'A',competition_eligible:false,registration_status:'not_in_current_competition_cohort',registration_evidence:{eligible:false},history:[{date:'2025-08-30',gameweek:4,minutes:0}],visible:false},
  {id:3,name:'Reserve only',club:'A',competition_eligible:false,registration_status:'not_in_current_competition_cohort',registration_evidence:{eligible:false},history:[],visible:false},
  {id:9,name:'Ambiguous',club:'A',competition_eligible:true,registration_status:'competition_eligible',registration_evidence:{eligible:true},history:[{date:'2025-08-30',gameweek:4}],visible:true}
 ]};
 ...
 assert.equal(payload.players[3].registration_status,'quarantined_current_squad_identity');
 assert.equal(payload.meta.fantasy_player_pool_policy.includes('registered Championship squad OR selected-league senior matchday evidence'),true);
}
{
...
console.log('squad/registration policy v3 regression tests passed');