...
(()=>{
  ...
  function injuryTerm(text){const m=String(text||'').match(/(?:injur(?:y|ed)|hamstring|knee|ankle|calf|groin|thigh|back|hip|shoulder|wrist|foot|heel|ligament|muscle|concussion|illness|virus|fracture|strain|sprain)(?:[ A-Za-z0-9'’\-]{0,42})?/i);return m?m[0].replace(/\s+/g,' ').trim():null}
  const injuryWords=['injur','hamstring','knee','ankle','calf','groin','thigh','shoulder','wrist','ligament','concussion','illness','fracture','strain','sprain'];
  function injuryTextHits(data,limit=12000){
    const letter=b=>{const c=b|32;return c>=97&&c<=122},hits=[];outer:for(let i=0;i<data.length;i++){const c=data[i]|32;if(c!==97&&c!==99&&c!==102&&c!==103&&c!==104&&c!==105&&c!==107&&c!==108&&c!==115&&c!==116&&c!==119)continue;for(const word of injuryWords){if(c!==word.charCodeAt(0))continue;let ascii=true,wide=true;for(let j=1;j<word.length;j++){if(i+j>=data.length||((data[i+j]|32)!==word.charCodeAt(j)))ascii=false;if(i+j*2>=data.length||data[i+j*2-1]!==0||((data[i+j*2]|32)!==word.charCodeAt(j)))wide=false;if(!ascii&&!wide)break}const asciiBefore=i===0||!letter(data[i-1]),asciiAfter=i+word.length>=data.length||!letter(data[i+word.length]),wideBefore=i<2||data[i-1]!==0||!letter(data[i-2]),wideAfter=i+word.length*2>=data.length||data[i+word.length*2-1]!==0||!letter(data[i+word.length*2]);ascii=ascii&&asciiBefore&&asciiAfter;wide=wide&&wideBefore&&wideAfter;if(ascii||wide){hits.push({at:i,wide});i+=ascii?word.length-1:word.length*2-1;if(hits.length>=limit)break outer;break}}}return hits;
  }
  function nearestPlayerId(data,ids,at,span=1800){
    const view=new DataView(data.buffer,data.byteOffset,data.byteLength),start=Math.max(0,at-span),end=Math.min(data.length-4,at+span);let best=null,bestDistance=Infinity;for(let off=start;off<=end;off++){const le=ids.get(view.getUint32(off,true)),be=ids.get(view.getUint32(off,false)),p=le||be;if(!p)continue;const distance=Math.abs(off-at);if(distance<bestDistance){best=p;bestDistance=distance;if(distance<12)break}}return best;
  }
  async function enrichPayloadFromInjuryTables(file,payload){
    ...
    try{
      ...
      const manifest=window.fzstd.decompress(tail.subarray(magic)),items=FM_RUNTIME.parseManifest(manifest).items.filter(m=>m.plain>0&&m.plain<=268435456&&(/(injur|medical|health|unavail|condition|physio|absence)/i.test(m.name)||/^(game_db|news)\.dat$/i.test(String(m.name||'').split(/[\\/]/).pop())));payload.meta=payload.meta||{};payload.meta.injury_members_scanned=items.map(x=>x.name);
      ...
      const latin=new TextDecoder('latin1'),utf16=new TextDecoder('utf-16le');let totalHits=0,linked=0;for(const item of items){let data;try{const src=b.subarray(26+item.offset,26+item.offset+item.stored);data=window.fzstd.decompress(src,new Uint8Array(item.plain))}catch(_e){continue}const hits=injuryTextHits(data);totalHits+=hits.length;for(const hit of hits){const p=nearestPlayerId(data,ids,hit.at);if(!p)continue;const key=String(p.pid??p.id??playerName(p)).toLowerCase();if(found.has(key))continue;const start=Math.max(0,hit.at-(hit.wide?24:12)),end=Math.min(data.length,hit.at+(hit.wide?240:120)),rawText=(hit.wide?utf16:latin).decode(data.subarray(start,end)).replace(/[^\x20-\x7e]+/g,' '),term=injuryTerm(rawText)||'Injured',title=term.charAt(0).toUpperCase()+term.slice(1);p.injury_status=title;found.set(key,{pid:String(p.pid??p.id??''),name:playerName(p),club:p.club,pos:p.pos,detail:/^injur/i.test(title)?title:`Injured · ${title}`});linked++}data=null}payload.meta.injury_text_hits=totalHits;payload.meta.injury_candidate_links=linked;
    }catch(e){console.warn('FM injury table enrichment skipped',e)}
    payload.injuries=[...found.values()];payload.meta=payload.meta||{};payload.meta.injury_records_decoded=payload.injuries.length;return payload;
  }
