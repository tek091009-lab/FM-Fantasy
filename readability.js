...
(()=>{
  ...
  function addFinalStyles(){
    ...
    const s=document.createElement('style');s.id='fmFinalPolishStyles';s.textContent=`
      ...
      #starAllToggle{min-width:42px;height:27px;margin-left:5px;padding:0 9px;border:1px solid rgba(255,255,255,.13);border-radius:8px;color:#d8cce8;background:#211440;font:800 9px/1 inherit;cursor:pointer}
  ...
  function collectPayloadInjuries(payload){
    const sources=[payload?.injuries,payload?.active_injuries,payload?.medical?.injuries,payload?.availability?.injuries,payload?.statuses?.injuries].filter(Array.isArray),players=payload?.players||[],out=[];for(const rows of sources)for(const row of rows){const x=normaliseInjuryRow(row,players);if(x)out.push(x)}return out;
  }
