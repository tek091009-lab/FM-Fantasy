...
async function applyCanonicalPayload(reason='canonical recovery'){
 ...
 if(existing){markActive();try{window.FMNewsPersistence?.recoverFromPayload?.(reason)}catch(_e){};return true}
 ...
 try{
  ...
  try{window.FMNewsPersistence?.recoverFromPayload?.(reason)}catch(_e){}
...
window.FMNewsWorldRecovery={version:VERSION,applyCanonicalPayload,markActive,status:()=>({version:VERSION,loading,applied,attempts,payloadReady:!!payloadRef()})};
...
window.addEventListener('fmworldloaded',()=>{markActive();setTimeout(()=>window.FMNewsPersistence?.recoverFromPayload?.('world loaded recovery'),0)});
window.addEventListener('fmcanonicalpublished',()=>{markActive();setTimeout(()=>window.FMNewsPersistence?.recoverFromPayload?.('canonical published recovery'),50)});
