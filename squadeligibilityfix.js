...
function fix(payload){
 ...
 for(const p of payload.players){
   ...
   if(p?.unresolved===true||blocked.has(id)){
     p.visible=false;p.competition_eligible=false;p.registration_status='quarantined_current_squad_identity';blockedCount++;continue;
   }
   if(cohortEligible){
     p.visible=true;p.competition_eligible=true;p.registration_status='competition_eligible';registered++;
   }else if(matchday){
     p.visible=true;p.competition_eligible=true;p.registration_status='senior_matchday_exception';
     p.registration_evidence={...(p.registration_evidence||{}),eligible:true,override:VERSION,reason:'Not found in registration cohort, but selected-league senior matchday evidence makes the player Fantasy-eligible.'};
     ...
     p.visible=false;p.competition_eligible=false;p.registration_status='reserve_or_u21_no_senior_evidence';hiddenReserve++;
