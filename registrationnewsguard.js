(()=>{
'use strict';
const VERSION='registration-news-v2-canonical-boundary-observer-free';
...
function installPublishGuard(){
 const c=window.FMCloud;if(!c||c.__registrationNewsV2||typeof c.publishWorld!=='function')return false;
 c.__registrationNewsV2=true;const original=c.publishWorld.bind(c);
...
function pruneTransfers(root=document){
 ...
 const allowed=allowedTransferNames(),rows=[...new Set([...scope.querySelectorAll('[data-news-text]'),...scope.querySelectorAll('.transfer')])];
 for(const row of rows){const text=norm(row.textContent||'');row.style.display=allowed.some(n=>text.includes(n))?'':'none'}
 const shown=rows.filter(r=>r.style.display!=='none').length,toggle=scope.querySelector('[data-news-toggle]');
 if(toggle){toggle.dataset.newsCount=String(shown);toggle.textContent=`View all ${shown}`;toggle.style.display=shown?'':'none'}
}
...
window.addEventListener('fmcloudready',()=>{installPublishGuard();setTimeout(refresh,0);setTimeout(refresh,500)});
let tries=0;const timer=setInterval(()=>{tries++;if(installPublishGuard()||tries>50)clearInterval(timer)},200);
document.addEventListener('click',e=>{const b=e.target.closest?.('button,a,[role="button"]');if(!b)return;const txt=norm(b.textContent||'');if(txt==='news'||b.dataset?.nav==='news'||b.getAttribute('data-page')==='news')setTimeout(refresh,60)},true);
window.addEventListener('fmworldloaded',()=>setTimeout(refresh,0));
})();