...
function apply(payload,old){
 ...
 const m=freezeHistoricalMatches(payload,old,oldDone),p=freezePlayerHistory(payload,old,oldDone);payload.meta=payload.meta||{};payload.meta.historical_match_freeze_v12={version:VERSION,policy:'same-world weekly updates keep all match and player history through the previously completed Gameweek exactly canonical; later saves may append only post-boundary history, never rewrite or backfill earlier fantasy scoring',old_completed_gameweek:oldDone,old_snapshot_date:String(old?.meta?.snapshot_date||'').slice(0,10)||null,...m,...p};payload.meta.historical_freeze_policy='canonical-through-previous-completed-gameweek-v12';return payload.meta.historical_match_freeze_v12;
}
function install(){
