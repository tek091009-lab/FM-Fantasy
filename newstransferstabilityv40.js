(()=>{
'use strict';
const VERSION='news-transfer-stability-v40-canonical-dom-authority';
...
function transferNeedsRepair(card){
 ...
 const host=card.querySelector('.fmCanonicalTransferRows'),events=transferEvents();
 if(!host)return true;
...
function registrationNeedsRepair(card){
 if(!card)return false;
 const rows=card.querySelector('.newsRegRows'),events=registrationEvents();
 if(!rows)return true;
...
new MutationObserver(muts=>{if(!busy&&muts.some(relevantMutation))stabilise()}).observe(document.documentElement,{subtree:true,childList:true,characterData:true});
...
window.FMNewsTransferStabilityV40={version:VERSION,stabilise,transferNeedsRepair,registrationNeedsRepair,hideLegacyTransferRows};
})();
