...
function seasonEvidence(p){
 const m=p?.meta||{};
 const ordered=[['season_start',m.season_start],['season_year',m.season_year],['season_name',m.season_name],['season',m.season]];
 for(const [field,value] of ordered){const y=yearFrom(value);if(y!==null)return {canonical_start_year:y,source_field:field,source_value:clean(value),confidence:'year_anchor'};}
...
function relation(next,old){
 ...
 return {same_competition:sameCompetition,season_known:bothCanonical,same_season:sameSeason,cross_season:crossSeason,next_competition:nc||null,old_competition:oc||null,next_season_start:ne.canonical_start_year,old_season_start:oe.canonical_start_year,next_season_evidence:ne,old_season_evidence:oe,policy:'cross-season only when both payloads expose disagreeing canonical start-year evidence; raw formatting and opaque season IDs cannot trigger a rollover'};
}
function install(){
 ...
 c.publishWorld=async(payload,...args)=>{
  ...
  if(payload&&typeof payload==='object'){payload.meta=payload.meta||{};payload.meta.season_continuity_v91={version:VERSION,...r};}
