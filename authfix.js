(()=>{
 ...
 async function signup(){
   ...
   let payload;
   try{
     const r=await fetch(`${cfg.supabaseUrl}/functions/v1/fmfantasy-signup`,{method:'POST',headers:{'Content-Type':'application/json','apikey':cfg.supabaseAnonKey},body:JSON.stringify({username,password,role,worldName,joinCode:code||null})});
     payload=await r.json();
     if(!r.ok||payload?.error)return msg(payload?.error||'Could not create account.');
   }catch(e){return msg(e?.message||'Could not create account.')}
   const {data,error}=await client.auth.signInWithPassword({email:synthetic(username),password});if(error)return msg(error.message);
   session=data.session;await hydrate();if(role==='creator'&&payload?.joinCode)alert(`Creator account ready. Your join code is ${payload.joinCode}`);
 }
