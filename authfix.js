(()=>{
 ...
 const cacheKey=id=>`fmFantasyWorldVersion:${id||''}`;
 const worldVersion=w=>`${String(w?.updated_at||'')}|${Number(w?.payload_version||0)}`;
 ...
 async function storeAuthoritativePayload(payload){
  if(!payload)return;
  try{
  ...
  }catch(e){console.warn('Could not refresh local FM world cache',e)}
 }
 ...
 async function hydrate(){
  ...
  const{data:w,error:we}=await client.from('worlds').select('id,creator_id,name,join_code,updated_at,payload_version').eq('id',m.world_id).single();if(we||!w){msg(we?.message||'Shared FM world could not be loaded.');showGate();return}world=w;
 ...
 async function refreshWorldStamp(){if(!client||!world?.id)return null;const{data,error}=await client.from('worlds').select('updated_at,payload_version').eq('id',world.id).single();if(!error&&data?.updated_at){world.updated_at=data.updated_at;world.payload_version=Number(data.payload_version||0);try{localStorage.setItem(cacheKey(world.id),worldVersion(world))}catch(_){}}return world.updated_at||null}
 async function publishWorld(payload){
  if(!profile||profile.role!=='creator'||!world)return null;
  const text=payload==null?null:JSON.stringify(payload);const{error}=await client.rpc('fmfantasy_publish_world',{p_world_id:world.id,p_payload_text:text});if(error)throw error;
  await refreshWorldStamp();
  if(payload==null){world.payload=null;return null}
  return await loadWorld(true);
 }
 async function loadWorld(force=false){
  if(!world)return null;
  const remoteVersion=worldVersion(world);let localVersion='';try{localVersion=localStorage.getItem(cacheKey(world.id))||''}catch(_){}
  if(!force&&remoteVersion&&localVersion===remoteVersion&&typeof window.fmStoredGet==='function'){
  ...
  const{data,error}=await client.from('worlds').select('payload,updated_at,payload_version').eq('id',world.id).single();if(error)throw error;
  let payload=data?.payload||null;if(data?.updated_at)world.updated_at=data.updated_at;world.payload_version=Number(data?.payload_version||0);
  if(!payload&&profile?.role==='creator'&&typeof window.fmStoredGet==='function'){const local=await window.fmStoredGet();if(local){await publishWorld(local);payload=world.payload}}
  if(payload){world.payload=payload;await storeAuthoritativePayload(payload);try{localStorage.setItem(cacheKey(world.id),worldVersion(world))}catch(_){}}
  return payload
 }
