(()=>{
'use strict';
const VERSION='impact-rotation-pricing-v2-core-chain-safe';
...
function install(){
  ...
  // CRITICAL: this is an add-on, never the core importer. It must wait until V85
  // discipline/pricing + V86 discipline reconciliation + GK pricing are genuinely installed.
  if(typeof original!=='function'||original.__fmImpactRotationV1||!original.__fmV85Wrapped||!original.__fmV86Post||!original.__fmV86Wrapped)return false;
  const wrapped=function(payload,...args){const out=original(payload,...args);transform(payload);try{if(typeof FM_DEBUG!=='undefined')FM_DEBUG.lastMeta=payload?.meta||null;if(typeof fmDebugAdd==='function')fmDebugAdd('info','High-impact rotation pricing finaliser applied.',{changes:payload?.meta?.impact_rotation_price_changes||0})}catch(_e){}return out};
  wrapped.__fmImpactRotationV1=true;wrapped.__fmV86Wrapped=true;wrapped.__fmV86Post=true;wrapped.__fmV85Wrapped=true;globalThis.fmApplyPostPayloadPricingCorrections=wrapped;return true
}
window.FMImpactRotationPricingV1={version:VERSION,transform,install};let tries=0;const timer=setInterval(()=>{tries++;if(install()||tries>240)clearInterval(timer)},50);install();
})();
