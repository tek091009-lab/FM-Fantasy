(()=>{
'use strict';
const VERSION='premium-defensive-midfielder-pricing-v2-core-chain-safe';
const num=v=>Number(v||0)||0;
const clamp=(v,a=0,b=1)=>Math.max(a,Math.min(b,num(v)));
const norm=v=>String(v??'').trim().toLowerCase();
function defconPer90(p){const mins=Math.max(0,num(p?.minutes));if(!mins)return 0;const total=(Array.isArray(p?.history)?p.history:[]).reduce((s,h)=>s+Math.max(0,num(h?.defcon_actions)),0);return total*90/mins}
function transform(payload){
  ...
  for(const p of payload.players){
    ...
    const c=p.price_context||{},role=norm(c.role);if(!role.includes('defensive midfielder'))continue;
    ...
    const currentStarter=starts>=.60||share>=.68;if(obs<3||!currentStarter||q<.55||team<.75||mins<=0||c.current_unavailable)continue;
    const d90=defconPer90(p);if(d90<9)continue;
    let target=6.0;if(team>=.90&&q>=.65&&d90>=10)target=6.5;else if(team>=.82&&q>=.65&&d90>=15)target=6.5;
...
function install(){
  ...
  // Final pricing add-on in this chain: require the defender add-on, which itself requires
  // rotation -> V86 GK -> discipline-post -> core V85. This prevents this module from
  // ever masquerading as the V85 importer and suppressing Rayan/Taty logic again.
  if(typeof original!=='function'||original.__fmPremiumDMV1||!original.__fmPremiumDefV1||!original.__fmImpactRotationV1||!original.__fmV85Wrapped||!original.__fmV86Post||!original.__fmV86Wrapped)return false;
  ...
  wrapped.__fmPremiumDMV1=true;wrapped.__fmPremiumDefV1=true;wrapped.__fmImpactRotationV1=true;wrapped.__fmV86Wrapped=true;wrapped.__fmV86Post=true;wrapped.__fmV85Wrapped=true;globalThis.fmApplyPostPayloadPricingCorrections=wrapped;return true
}
window.FMPremiumDMPricingV1={version:VERSION,transform,install};let tries=0;const timer=setInterval(()=>{tries++;if(install()||tries>240)clearInterval(timer)},50);install();
})();
