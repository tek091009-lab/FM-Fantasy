...
function build(){
 ...
 const namingKeys=['legal_name','legal_full_name','full_name','first_name','forename','surname_name','football_surname','surname','family_name','last_name','common_name','known_as','preferred_name','nickname','shirt_name','preferred_short_name','public_name','canonical_display_name','football_display_name','display_name','name'];
 const identityKeys=['id','player_id','uid','person_id','club_id','club','pos','position','price','value'];
 const availabilityKeys=['injury_status','injury_name','injury_type','injury_detail','expected_return_date','injury_return_date','injured_until','injury_days_remaining','suspension_status','suspension_remaining','suspension_games_remaining','ban_remaining','ban_games','banned_until','suspension_detail'];
 const formKeys=['form','form_points','weekly_points','history','retained_history_evidence','discipline_evidence'];
 ...
 const commonSurnameValidated=visible.filter(p=>p?.name_component_evidence?.common_plus_surname_is_validated_by_display).length;
 ...
 const capabilities={
   ...
   availability:{explicit_evidence_players:availabilityKnown,unknown_players:availabilityUnknown,discipline_history_players:disciplinePlayers,field_coverage:availabilityCov},
   naming:{field_coverage:nameCov,common_plus_surname_validated:commonSurnameValidated},
   form:{field_coverage:formCov},
 ...
 if(availabilityUnknown===visible.length&&visible.length)gaps.push('current_injury_suspension_state');
