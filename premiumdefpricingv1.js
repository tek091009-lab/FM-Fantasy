(()=>{
'use strict';
const VERSION='premium-attacking-defender-pricing-v2-core-chain-safe';
...
function install(){
  ...
  // Must sit after the core V85/V86 chain and the rotation add-on. Never fake core markers.
  if(typeof original!=='function'||original.__fmPremiumDefV1||!original.__fmImpactRotationV1||!original.__fmV85Wrapped||!original.__fmV86Post||!original.__fmV86Wrapped)return false;
  ...
  wrapped.__fmPremiumDefV1=true;wrapped.__fmImpactRotationV1=true;wrapped.__fmV86Wrapped=true;wrapped.__fmV86Post=true;wrapped.__fmV85Wrapped=true;globalThis.fmApplyPostPayloadPricingCorrections=wrapped;return true
}
window.FMPremiumDefenderPricingV1={version:VERSION,transform,install};let tries=0;const timer=setInterval(()=>{tries++;if(install()||tries>240)clearInterval(timer)},50);install();
})();
