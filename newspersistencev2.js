(()=>{
'use strict';
const VERSION='news-persistence-v4-refresh-safe-import-only';
...
let queueInstalled=false,storeInstalled=false,restoring=false,committed=null,pendingImport=null,commitWindowUntil=0,payloadSeenAt=0;
...
function chooseSnapshot(){
 ...
 const local=readLocal(),canon=canonicalSnapshot(payload);
...
function restoreDom(dom){
 ...
 const stamp=document.getElementById('newsStamp');
 if(stamp&&dom.stamp&&stamp.textContent!==dom.stamp){stamp.textContent=dom.stamp;changed=true}
...
function applyStateNews(news){
 ...
 const st=stateRef();
 if(st&&JSON.stringify(st.news)!==JSON.stringify(news)){st.news=clone(news);changed=true}
...
function restore(){
 ...
 // Startup is not a database clear. Never delete the import-bound snapshot just because
 // FMCloud has created the world shell before its payload has finished loading.
...
function persistFinalImportSnapshot(){
 ...
 try{
  const p=payloadRef();
  if(p?.meta?.news_snapshot_v1){p.meta.news_snapshot_v1.state_news=clone(stateNews);p.meta.news_snapshot_v1.global_news=clone(globalNews)}
 }catch(_e){}
}
...
function armFinalCapture(ms=8000){
 commitWindowUntil=Math.max(commitWindowUntil,Date.now()+ms);
 for(const delay of [0,100,250,500,900,1500,2500,4000,6000,7800])setTimeout(persistFinalImportSnapshot,delay);
 setTimeout(()=>{commitWindowUntil=0;restore()},ms+250);
}
function installStoreWrapper(){
 ...
 const wrapped=async function(payload){
  ...
  // A transient null during startup/sync is not permission to destroy the saved news.
  // Explicit season clear already removes FM/news storage through clearflowv2.
  if(payload==null)return original(payload);
 ...
 wrapped.__fmNewsPersistenceWrapped=true;
 wrapped.__fmNewsPersistenceOriginal=original;
...
function installQueueGuard(){
 ...
 if(!c||typeof c.queueManagerSave!=='function'||c.__newsPersistenceQueueV1)return false;
...
window.addEventListener('fmcanonicalpublished',()=>{
 if(pendingImport){committed=pendingImport;writeLocal(committed);armFinalCapture()}
});
document.addEventListener('click',e=>{
 const b=e.target.closest?.('button,a,[role="button"]');if(!b)return;
 const s=norm(b.textContent||b.dataset?.nav||b.getAttribute('data-page')||'');
 if(s.includes('news'))scheduleRestore();
},true);
let tries=0;
const timer=setInterval(()=>{
 tries++;installQueueGuard();installStoreWrapper();
 const p=payloadRef();
 if(p&&!importMode()&&Date.now()>=commitWindowUntil){if(!payloadSeenAt)payloadSeenAt=Date.now();restore()}
 // Do not stop merely because the wrappers installed. Keep the restore loop alive until
 // the real payload has been present for a grace period, so refresh/load ordering cannot lose news.
 if(queueInstalled&&storeInstalled&&payloadSeenAt&&Date.now()-payloadSeenAt>6000)clearInterval(timer);
